...
namespace QRTracking
{
    ...
    public class QRCode : MonoBehaviour
    {
        ...
        private GameObject qrCodeCube = null;
        ...
        void Start()
        {
            ...
            qrs = GameObject.FindGameObjectsWithTag("QRSquare");

            //this happens when each qr code is initialized, but when it gets updated, its different. you may need this code in the updated section as well. or some variation
        ...
        void UpdatePropertiesDisplay()
        {
            ...
            if (qrCode != null && lastTimeStamp != qrCode.SystemRelativeLastDetectedTime.Ticks)
            {
                ...
                if (qrCodeCube != null  && set)
                {
                    qrCodeCube.transform.localPosition = new Vector3(PhysicalSize / 2.0f, PhysicalSize / 2.0f, 0.0f);
                    qrCodeCube.transform.localScale = new Vector3(PhysicalSize / 2.0f, PhysicalSize / 2.0f, 0.005f);
                    set = false;
                }
                

                //sprite.gameObject.transform.localScale = new Vector3(PhysicalSize, PhysicalSize, 0.005f); //size of qr sprite
                
                //sprite.transform.localPosition = new Vector3(PhysicalSize / 2.0f, PhysicalSize / 2.0f, 0.0f);
